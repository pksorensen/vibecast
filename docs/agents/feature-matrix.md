...
| Permission bypass | `--dangerously-skip-permissions` (+ auto-answer of its confirm dialog) | `approval_policy=on-request` + PreToolUse guard hook (the floor) + `-s danger-full-access`. The OS sandbox is OFF because `workspace-write` can't init in the runner container (userns/uid_map denied → C05 fails); the guard hook still fires under danger-full-access. ⚠ `--dangerously-bypass-approvals-and-sandbox` is **forbidden** — it ALSO kills approvals (golden-test invariant; adapter-spec §4). No `--full-auto` in 0.142.5 | none needed — pi has **no** permission system (YOLO by design) |
