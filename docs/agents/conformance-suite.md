...
| C05 | tool-events | tool_calls | R M (M = scripted write tool call) | prompt = "create file <nonce>.txt with content <nonce>" → `tool_use` and `tool_use_end` with a write-class toolName and matching `toolUseId` pair; file exists with content |
